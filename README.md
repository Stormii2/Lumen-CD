...
Target output once scope resolution lands (tracked in the roadmap below):

$ lumenc examples/demo.lum
error[E0412]: cannot find variable `totl` in this scope
--> examples/demo.lum:7:13
  |
5 | int total = 0;
  |     ----- a similar name is declared here
6 |
7 | count = totl + 1;
  |         ^^^^ not found in this scope
  |
help: did you mean `total`?
note: 1 error generated; 1 fix available (--fix)

The current build's actual runnable output — lexer + parser diagnostics only — is shown in Running It below.
